...
- **Early-end vote** — anyone can propose ending the game early, decided by a vote counted against **connected players** (with a cooldown after a reject). The vote itself is a **non-blocking chip**, never a modal — the table stays fully playable while it's open, so one idle voter can't freeze everyone. Round progression isn't host-only either.
...
Every fixed journey bug gets pinned as an e2e test, and **every shipped game has its own journey spec** — including negative assertions that secret information (roles, hands, the liar's word) never appears on the wrong player's screen. `e2e/tests/home-enter.spec.ts` (Enter intent), `lobby-copy.spec.ts` (clipboard), and `rummikub.spec.ts` (gestures, snap-back, device gate) set the precedent.
